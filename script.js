...
setInterval(() => {
 ...
 $(document).on("click", ".stripe1", event=>{
    console.log("stripe1")

     event.preventDefault();
     event.stopPropagation();
     if($(event.target).attr("class")=="stripe1"){
     var newsNumber = $(event.target).attr("number");
     openScreen(newsNumber);
    }
 })
 $(document).on("click", ".stripe2", event=>{
     console.log("stripe2")
    event.preventDefault();
    event.stopPropagation();
    if($(event.target).attr("class")=="stripe2"){
    var newsNumber = $(event.target).attr("number");
    openScreen(newsNumber);
   }
})
$(document).on("click", ".stripe3", event=>{
    console.log("stripe3")
    event.preventDefault();
    event.stopPropagation();
    if($(event.target).attr("class")=="stripe3"){
    var newsNumber = $(event.target).attr("number");
    openScreen(newsNumber);
   }
})
function eraseSky(color){
    console.log(color)
    var stripes = $(".skyStripe");
    for(let i=0;i<stripes.length;i++){
        setTimeout(() => {
            $(stripes[i]).css("background-color",'black');
            $('.ground').css("background-image","linear-gradient(to bottom,"+ color+" "+(50-(i+1)*20)+"%, black )")
...
function createSky(){
    ...
    var stripes = $(".skyStripe");
   for(let i=1;i<stripes.length+1;i++){
       setTimeout(() => {
           
       
       $(stripes[i-1]).css("background-color",color);
       $(stripes[i-1]).css("opacity", 1/(1+.2*i));
       $(".ground").css("background-image","linear-gradient(to bottom,"+ color+" "+ i*10+"%, black)")
